...
class FuelProductAdapter(private val listener: SelectFuelProductListener?) : ListAdapter<FuelProductDisplayModel, FuelProductViewHolder>(
    object :
        DiffUtil.ItemCallback<FuelProductDisplayModel>() {
        override fun areItemsTheSame(oldItem: FuelProductDisplayModel, newItem: FuelProductDisplayModel) =
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FuelProductViewHolder =
        LayoutInflater.from(parent.context).run {
            FuelProductViewHolder(inflate(R.layout.fuel_product_list_item, parent, false), listener)
        }
